  ...
  <div style="padding-bottom: 20px">
    <div style="margin: 10px 0">
      <div style="width: 100%; display: flex">
        <div style="width:100%">
          <video style="width: 100%" autoplay="autoplay" controls="controls" loop="loop"
            src="../../assets/my.mp4">
            <source
              src="../../assets/my.mp4"
              type="video/mp4">
          </video>
          <!-- <video style="width: 100%" src="https://www.bilibili.com/video/BV1iL4y1j7YJ/?spm_id_from=333.337.search-card.all.click" muted="true" autoplay="autoplay"  controls="controls" loop="loop"></video> -->
        </div>
      </div>
    </div>
    ...
    <div style="margin: 10px 0; padding: 20px; background-color: #fff; border-radius: 10px">
      ...
      <el-collapse v-model="active">
        <el-collapse-item :name="index + ''" v-for="(item, index) in notices">
          ...
          <div style="color: #888;text-indent: 1.5em;">{{ item.content }}</div>
        </el-collapse-item>
      </el-collapse>
    </div>
    ...
    <div style="margin: 10px 0">
      <div style="padding: 20px 10px; font-size: 28px; background-color: #fff; border-radius: 10px; color: #3F5EFB">流浪宠物
      </div>
      <el-card v-for="item in tableData" :key="item.id" style="margin: 10px 0">
        <div style="display: flex">
          <div style="width: 140px" @click="$router.push('/front/homeDetail?id=' + item.id)"> <img :src="item.img" alt=""
              style="width: 100%; height: 160px; border-radius: 10px"></div>
          <div style="padding-left: 50px; flex: 1">
            <div style="border-bottom: 1px solid #ddd; width: 100%; padding-bottom: 10px">
              ...
              <span style="margin-left: 20px">{{ item.age }}</span>岁
            </div>
